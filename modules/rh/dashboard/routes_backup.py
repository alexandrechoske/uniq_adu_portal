...
- 4 Gráficos: Evolução Headcount, Admissões vs Desligamentos, Turnover por Departamento, Vagas Abertas
"""
...
def calcular_kpis(periodo_inicio, periodo_fim, departamentos_ids=None):
    """
    Calcula os 5 KPIs principais do Dashboard Executivo
    ...
    5. Custo Total - Soma dos salários mensais
    """
    print("🚀 Calculando KPIs do Dashboard Executivo...")
    ...
    # KPI 5: Custo Total de Pessoal
    kpis['custo_total'] = calcular_kpi_custo_total()
...
def calcular_kpi_turnover(periodo_inicio, periodo_fim, headcount_atual):
    ...
    try:
        ...
        print(f"   ✅ Turnover: {turnover_taxa:.1f}% ({desligamentos} desligamentos)")
...
def calcular_kpi_tempo_contratacao(periodo_inicio, periodo_fim):
    ...
    try:
        ...
        for vaga in vagas:
            if vaga.get('data_abertura') and vaga.get('data_fechamento'):
                try:
                    ...
                    if dias >= 0:  # Ignorar datas inválidas
                        total_dias += dias
                        count_vagas += 1
                except:
                    continue
        ...
        print(f"   ✅ Tempo Médio Contratação: {tempo_medio} dias ({count_vagas} vagas)")
...
def calcular_kpi_vagas_abertas():
    ...
    try:
        ...
        return {
            ...
            'cor': '#ffc107' if vagas_abertas > 5 else '#17a2b8'
        }
    except Exception as e:
...
def calcular_kpi_custo_total():
    """
    KPI 5: Custo Total de Pessoal (Folha Salarial Mensal)
    
    Lógica:
    - Para cada colaborador ativo, buscar último salário no histórico
    - Otimização: Buscar todos os salários de uma vez usando IN
    """
    try:
        ...
        if not colaboradores_ids:
            print(f"   ⚠️  Custo Total: Nenhum colaborador ativo")
            return {
                'valor': 0,
                'variacao': 0,
        ...
        # Mapear último salário de cada colaborador
        ...
        custo_total = sum(salarios_por_colaborador.values())
        ...
        print(f"❌ Erro ao calcular custo total: {str(e)}")
        import traceback
        traceback.print_exc()
        return {
            'valor': 0,
