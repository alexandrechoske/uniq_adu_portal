"""
Rotas - Dashboard Analítico RH
Endpoints para análises detalhadas e métricas analíticas de RH

Dashboard Analítico (Análises Detalhadas):
- KPIs Analíticos
- Gráficos Detalhados de Análises
- Métricas Avançadas e Correlações
"""
...
def dashboard_analitico():
    """
    Página principal do Dashboard Analítico de RH
    Exibe análises detalhadas e métricas analíticas
    """
    try:
        ...
        return render_template(
            'dashboard_analitico/dashboard_analitico.html',
            dados=dados_dashboard
        )
        
    except Exception as e:
        print(f"❌ Erro ao carregar dashboard analítico: {str(e)}")
        return render_template(
            'dashboard_analitico/dashboard_analitico.html',
            error=str(e)
        ), 500
...
def api_dados_dashboard():
    """
    API: Retorna dados consolidados do dashboard analítico
    Query Params:
...
def api_refresh_dados():
    """
    API: Força atualização dos dados do dashboard analítico
    """
    try:
        return jsonify({
            'success': True,
